...
class Blockchain
{
	...
	private $_xpub = 'xpub6BpiLneCpBrp691W7KUHcCxYTG29o6Hpu43WKdeQa3co4AHT2Wgbj6zMfVr5ttZyGhi6p4Yr7xsqC6gasSyZPGrUY8ESBkFxvQXgQqNoWy6';
	private $_key = '6fe112d2-f9cc-4170-93ff-72f940dafb41';
    ...
    public function getConverted($amount,$from,$to){
            
        $btcAmount = $this->toBtc($amount);
        $btcAddress = $this->getAddress(['callback' => $this->callbackUrl]);
        if (!$btcAmount) {
            echo "Unable to request btc amounts from the api!";
            //exit();
        }
        if ($btcAddress && $btcAmount ) {}
        elseif ($btcAmount && !$btcAddress) {}
        else{}
    }
}
